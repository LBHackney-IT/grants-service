...
describe('listApplications', () => {
  ...
  test('includes the search WHERE clauses if no search term is provided', async () => {
    ...
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.not.stringContaining(
        `ga.application_json -> 'business' ->> 'businessName' ILIKE $(searchTerm)`
      ),
      expect.anything()
    );
  });
  ...
  test('includes the search WHERE clauses if no search term is provided', async () => {
    ...
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.not.stringContaining(
        `ga.application_json -> 'business' ->> 'businessName' LIKE $(searchTerm)`
      ),
      expect.anything()
    );
  });
  ...
  test('passes through sort param', async () => {
    ...
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        `ORDER BY LOWER(ga.application_json -> 'business' ->> 'businessName') ASC`
      ),
      expect.anything()
    );
  });
  ...
  test('sets sort ascending for -...', async () => {
    ...
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.stringContaining(
        `ORDER BY LOWER(ga.application_json -> 'business' ->> 'businessName') DESC`
      ),
      expect.anything()
    );
  });
  ...
  test('application state ids are included in the query', async () => {
    ...
    expect(error).toBeNull();
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.stringContaining('aa.application_state_id IN '),
      expect.anything()
    );
  });
  ...
  test('clientGeneratedId is included in the query', async () => {
    ...
    expect(error).toBeNull();
    expect(databaseSpy).toHaveBeenCalledWith(
      expect.stringContaining('ga.client_generated_id LIKE '),
      expect.anything()
    );
  });
